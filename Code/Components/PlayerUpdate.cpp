...
void CPlayerComponent::Update(float frameTime)
{
	...
	{
		gEnv->pRenderer->GetIRenderAuxGeom()->DrawLine(hit.pt - Vec3(.0f, .0f, 1.0f), ColorB(255, 0, 0), hit.pt + Vec3(.0f, .0f, 1.0f), ColorB(255, 0, 0), 6.f);
		gEnv->pRenderer->GetIRenderAuxGeom()->DrawLine(hit.pt - Vec3(.0f, 1.0f, .0f), ColorB(255, 0, 0), hit.pt + Vec3(.0f, 1.0f, .0f), ColorB(255, 0, 0), 6.f);
		gEnv->pRenderer->GetIRenderAuxGeom()->DrawLine(hit.pt - Vec3(1.0f, .0f, .0f), ColorB(255, 0, 0), hit.pt + Vec3(1.0f, .0f, .0f), ColorB(255, 0, 0), 6.f);
			...
			if (IEntity *pColliderEntity = gEnv->pEntitySystem->GetEntityFromPhysics(hit.pCollider))
			{
				if (SItemComponent *pColliderItem = pColliderEntity->GetComponent<SItemComponent>())
				{
					if(pColliderItem->IsPickable())
						ShowPickupMessage(pColliderItem->GetItemName());
				}
			}
		}
	}
